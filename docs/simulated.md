...
When commanded to a joint configuration, the simulated arm interpolates its joints toward the target over time at a configurable speed, like rdk's builtin `simulated` arm. It also serves 3D meshes for each link, so it renders as an SO-101 in the visualizer.
...
| `orientation_tolerance_deg` | float | Optional | Half-angle, in degrees, of the cone of acceptable end-effector approach directions around the goal orientation, used by `MoveToPosition`. **Roll about that axis is NOT constrained.** Range `[0, 180]`; `0`/unset means `30`. See [Approach-axis orientation planning](arm.md#approach-axis-orientation-planning). |
| `position_tolerance_mm` | float | Optional | Per-axis positional leeway, in millimetres, of the `MoveToPosition` goal cloud. Default `1.0`. See [Approach-axis orientation planning](arm.md#approach-axis-orientation-planning). |
