	...
	try {
		...
		if (!machineId) {
			return {
				...
				machineId: null,
				error:
					'Machine ID not found in URL path. Expected format: /machine/{machine-name} or /machine/{machine-name}/robot/{machine-id} or /robot/{machine-id}'
			};
		}
		...
		if (
			...
			!connectionDetails.apiKey.id ||
			!connectionDetails.apiKey.key
		) {
			return {
				connectionDetails: null,
				machineId,
...
/**
 * Get the machine root path for navigation
 * Returns the root path for the current machine (e.g., '/machine/laptop-main.kw8sl6xdac.viam.cloud/')
 */
export function getMachineRootPath(): string {
