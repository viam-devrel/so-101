...
| `mesh_decimation_ratios` | []float | Optional | Per-collision-mesh simplification ratios, one per URDF link in document order, each in `[0, 1]`. Only used when `use_urdf` is `true`. The bundled collision meshes already ship pre-decimated, so this normally stays empty (no runtime decimation); runtime mesh decimation is too slow to use routinely. Default `[]` (empty). |
