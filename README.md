...
| `use_urdf`         | bool     | Optional     | When `true`, sources kinematics and collision geometry from the bundled `arm/so101.urdf` instead of the embedded `so101.json`. Requires the `VIAM_MODULE_ROOT` environment variable to be set (viam-server sets this automatically). This is a drop-in swap: the URDF's `tool` frame is grafted to match `so101.json` exactly, so the TCP pose and kinematics are identical — only the collision geometry upgrades from primitive shapes to per-link meshes. Default `false`. |
| `mesh_decimation_ratios` | []float | Optional | Per-collision-mesh simplification ratios, one per arm link in document order (base, shoulder, upper_arm, lower_arm, wrist), each in `[0, 1]`. Only used when `use_urdf` is `true`. Each link ships a single merged collision mesh that is already pre-decimated, so this normally stays empty (no runtime decimation); runtime mesh decimation is too slow to use routinely. Default `[]` (empty). |
